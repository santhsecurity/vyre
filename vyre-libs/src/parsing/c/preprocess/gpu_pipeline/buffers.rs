...
mod generated_buffer_codec_tests {
    ...
    fn generated_words(seed: u32, count: usize) -> Vec<u32> {
        ...
        out.try_reserve_exact(count)
            .expect("generated test word storage must reserve");
    ...
    fn generated_u32_pack_prefix_decode_round_trips_4096_shapes() {
        for seed in 0..4096u32 {
            ...
            let packed =
                pack_u32_words(&words, pad_len).expect("generated u32 table packing must not fail");
            ...
            .expect("generated packed table must decode");
            ...
            let over_requested = unpack_u32_words_prefix(&packed, table_words + 7)
                .expect("over-requested prefix decode must clamp to available words");
            assert_eq!(
                over_requested.len(),
                table_words,
    ...
    fn generated_byte_padding_preserves_prefix_and_zero_fills_4096_lengths() {
        for len in 0..4096usize {
            ...
            bytes
                ...
                .expect("generated byte storage must reserve");
            for index in 0..len {
                bytes.push((index as u8).wrapping_mul(37).wrapping_add(len as u8));
            }

            let padded = pad_to_u32_words(&bytes).expect("generated byte padding must not fail");
            assert_eq!(
                padded.len() % 4,
                0,
        ...
        for seed in 0..1024u32 {
            ...
            assert!(
                unpack_u32_words_prefix_exact(&trailing, word_count, word_count, "trailing table")
                    .is_err(),
                "seed {seed}: trailing-byte exact table must be rejected"
            );
        }
    }
}
