...
- Release notes must use `vyre 0.6.1` and `weir 0.1.0`.
- Release notes must state that `vyre`, `vyre-driver-cuda@0.6.1`, `vyre-driver-wgpu@0.6.1`, `vyrec`, and `vyre-frontend-c` are present on the `0.6.1` Vyre release train, and `weir` is present at `0.1.0`; `missing_required_release_packages` in `version-matrix.json` must be empty before notes are cut.
...
- Release notes must reference RC tags `vyre-v0.6.1-rc.1`, `weir-v0.1.0-rc.1`, and `vyre-0.6.1-weir-0.1.0-rc.1` before final tags `vyre-v0.6.1`, `weir-v0.1.0`, and `vyre-0.6.1-weir-0.1.0`.
- Release notes must not instruct a bare `v0.6.1` tag workflow.
