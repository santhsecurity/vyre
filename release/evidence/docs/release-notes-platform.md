...
- Release notes must use `vyre 0.4.2` and `dataflow-consumer 0.1.0`.
- Release notes must state that `vyre`, `vyre-driver-cuda@0.4.2`, `vyre-driver-wgpu@0.4.2`, `vyrec`, and `vyre-frontend-c` are present on the `0.4.2` Vyre release train, and `dataflow-consumer` is present at `0.1.0`; `missing_required_release_packages` in `version-matrix.json` must be empty before notes are cut.
...
- Release notes must reference RC tags `vyre-v0.4.2-rc.1`, `dataflow-consumer-v0.1.0-rc.1`, and `vyre-0.4.2-dataflow-consumer-0.1.0-rc.1` before final tags `vyre-v0.4.2`, `dataflow-consumer-v0.1.0`, and `vyre-0.4.2-dataflow-consumer-0.1.0`.
