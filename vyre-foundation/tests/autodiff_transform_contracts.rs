...
fn square_via_local_program() -> Program {
    Program::wrapped(
        vec![
...
fn generated_intermediate_buffer_program(seed: u32) -> Program {
...
fn generated_f32_identity_cast_program(seed: u32) -> Program {
    let gid = Expr::InvocationId { axis: 0 };
...
fn generated_nondifferentiable_cast_shape(seed: u32) -> (DataType, DataType) {
...
fn generated_cast_program(source: DataType, target: DataType) -> Program {
    Program::wrapped(
        vec![
            BufferDecl::storage("x", 0, BufferAccess::ReadOnly, source).with_count(1),
...
fn generated_differentiable_program(seed: u32) -> Program {
    let gid = Expr::InvocationId { axis: 0 };
    let x = Expr::load("x", gid.clone());
    let w = Expr::load("w", gid.clone());
