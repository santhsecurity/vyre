...
fn source_files_under(root: &Path, out: &mut Vec<PathBuf>) {
    let entries = fs::read_dir(root).unwrap_or_else(|error| {
        panic!(
            "failed to read vyre-primitives source directory {}: {error}",
            root.display()
        )
    });
    ...
    for entry in entries {
        let entry = entry.unwrap_or_else(|error| {
            panic!(
                "failed to read vyre-primitives source directory entry under {}: {error}",
                root.display()
            )
        });
        let path = entry.path();
        let file_type = entry.file_type().unwrap_or_else(|error| {
            panic!(
                "failed to classify vyre-primitives source path {}: {error}",
                path.display()
            )
        });
        if file_type.is_dir() {
...
fn primitive_source_does_not_name_downstream_consumers() {
    ...
    for source_file in source_files {
        let contents = fs::read_to_string(&source_file).unwrap_or_else(|error| {
            panic!(
                "failed to read vyre-primitives source file {}: {error}",
                source_file.display()
            )
        });
    ...
    assert!(
        violations.is_empty(),
        "vyre-primitives is the reusable primitive authority and must not name downstream consumers:\n{}",
        violations.join("\n")
    );
}
