...
fn cuda_per_word_popcount_matches_cpu_small() {
    ...
    let gpu = per_word_popcount_via(&dispatcher, &input).expect("dispatch");
...
fn cuda_per_word_popcount_handles_empty() {
    let backend = live_dispatcher();
    let dispatcher = CudaOptimizerDispatcher { backend: &backend };
    let gpu = per_word_popcount_via(&dispatcher, &[]).expect("dispatch");
    assert!(gpu.is_empty());
}
...
fn cuda_total_set_bits_via_matches_cpu() {
    ...
    let input: Vec<u32> = (0..512)
        ...
        .collect();
    let gpu = total_set_bits_via(&dispatcher, &input).expect("dispatch");
...
fn cuda_saturation_ratio_via_matches_cpu() {
    ...
    let gpu = saturation_ratio_via(&dispatcher, &input).expect("dispatch");
