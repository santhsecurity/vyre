...
fn cuda_semiring_gemm_bool_or_matches_reference_3x3_identity() {
    ...
    let b = a.clone();
    let gpu = semiring_gemm_via_bool_or(&dispatcher, &a, &b, 3, 3, 3).expect("dispatch");
...
fn cuda_semiring_gemm_bool_or_chain_reach() {
    ...
    let gpu = semiring_gemm_via_bool_or(&dispatcher, &a, &b, 4, 4, 4).expect("dispatch");
...
fn cuda_semiring_gemm_min_plus_matches_reference() {
    ...
    let b = a.clone();
    let gpu = semiring_gemm_via_min_plus(&dispatcher, &a, &b, 4, 4, 4).expect("dispatch");
...
fn cuda_semiring_gemm_lineage_matches_reference() {
    ...
    let b = a.clone();
    let gpu = semiring_gemm_via_lineage(&dispatcher, &a, &b, 4, 4, 4).expect("dispatch");
...
fn cuda_semiring_gemm_bool_or_random_8x8() {
    ...
    let gpu = semiring_gemm_via_bool_or(&dispatcher, &a, &b, m, n, k).expect("dispatch");
